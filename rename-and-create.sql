-- ══════════════════════════════════════════
-- STEP 1: 기존 테이블 이름 변경 (안전하게 보존)
-- ══════════════════════════════════════════

ALTER TABLE IF EXISTS instructors RENAME TO instructors_old;
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'school_id_enum') THEN
