...
export async function generateMarketAnalysis(
  ...
  if (isGeminiEnabled()) {
    try {
      const prompt = `당신은 한국 시장 분석 전문가예요.
다음 정보를 바탕으로 시장 분석을 해주세요.
...
- analysisReport: 위 분석 결과를 종합한 20줄 분석 레포트 (\\n으로 줄바꿈). 키워드 트렌드, 경쟁사 현황, 고객 니즈를 정리하고, 이 시장에 들어갈 때 주의할 점과 기회를 설명해주세요.
- 아이템 형태(${itemType})에 맞는 시장 분석을 해주세요
- 모든 텍스트는 외국인이 알아들을 수 있는 쉬운 한국어 (~해요 체)
...
export async function generateBrandingStrategy(
  ...
  if (isGeminiEnabled()) {
    try {
      ...
      const prompt = `당신은 한국 브랜딩 전략 전문가예요.
경쟁사 정보, 고객의 불만, 나의 강점을 바탕으로 브랜딩 전략을 만들어주세요.
...
- brandingReport는 20줄(\\n으로 줄바꿈)로:
  ...
  * 우리가 경쟁사보다 잘할 수 있는 점
  * USP를 이렇게 정한 이유
  * 브랜드 이름, 슬로건, 무드를 이렇게 정한 이유를 친절하게 설명
- 모든 텍스트는 외국인이 알아들을 수 있는 쉬운 한국어 (~해요 체)`;
